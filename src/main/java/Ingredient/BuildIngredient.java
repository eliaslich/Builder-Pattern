...
public class BuildIngredient {
    ...
    private final static List<String> NEW_EXCLUDE_WORDS = ImmutableList.of("juice of", "juice from", "crushed",
            "heaped", "Shredded", "cut in half",
            "Sliced", "toasted", "organic", "finely", "finely", "chopped", "Washed", "Halved", "Minced", "Cut",
            "chunks", "chunk",
            "Thinly sliced", "Roughly chopped", "Chopped", "sliced", "rinsed", "in half", "Cubed", "Steamed",
            "Spiralized", "Ribboned",
            "Deseeded", "for serving", "for frying", "dried", "grated", "packed", "for cooking", "for serving",
            "ready to eat", "into ", " bite ", "size",
            "of ", "(optional)", "(for frying)", "optional", "about", "ripe", "or ", "cubes", "frozen", "possible",
            "possibly", "one ", "for "); // deleted "pure" since it ruined puree, added "one", "bite", "chunk(s)",
                                         // "into", "size"

    private final static List<String> PER_QUANTITY_WORDS = ImmutableList.of("each", "per", "á", "à", "correspond",
            "corresponds");
    ...
    void ParseIngredient(final String ingredientDisplayLine) {
        ...
        if (ingredientDisplayLine.length() > 0) {
            ingredient = new Ingredient(ingredientFoodParsedString, quantity, unit, ingredientDisplayLine,
                    lastIngredientTag);
    ...
    Optional<MeasurementUnit> ParseUnit(String toParse, List<String> remainingData, boolean recursive) {
        ...
        for (String unitParse : toParseUnit) {
            ...
            if (unitFound.isPresent()) {
                unit = unitFound.get();
                if (!recursive)
                    safeRemoveIngredient(remainingData, unitParse);
                parsed = true;
                break;
            }
        }
    ...
    /**
     * This method is called (hopefully) after the amount and measure unit has been
     * extracted, excluded words have been removed.
